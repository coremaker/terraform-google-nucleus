...
resource "google_dns_record_set" "dns_record" {
  ...
  rrdatas      = each.value == "istio-ingress" ? [google_compute_address.istio_gateway_ip.0.address] : [google_compute_global_address.namespace_public_ip[each.value].address]
