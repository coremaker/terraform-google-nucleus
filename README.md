...
| <a name="input_k8s_namespaces"></a> [k8s\_namespaces](#input\_k8s\_namespaces) | List of namespaces to be created. | <pre>list(object({<br>    name          = string<br>    has_public_ip = optional(bool, false)<br>    regional_ip   = optional(bool, false)<br>    dns_records   = optional(set(string))<br>    labels        = optional(map(string))<br>    annotations   = optional(map(string))<br>  }))</pre> | n/a | yes |
